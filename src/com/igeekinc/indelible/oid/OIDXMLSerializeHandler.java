...
public class OIDXMLSerializeHandler implements XMLObjectSerializeHandler
{

    public void serializeObject(String fieldName, ContentHandler xmlHandler,
            Object objectToSerialize, PauserControlleeIF pauser)
            throws SAXException, AbortedException, IOException
    {
        if (pauser != null)
            pauser.checkPauseAndAbort();
        xmlHandler.startElement("", "", fieldName, new AttributesImpl()); //$NON-NLS-1$ //$NON-NLS-2$
        String serializeString = ((ObjectID)objectToSerialize).toString();
        
        char [] outputChars = serializeString.toCharArray();
        xmlHandler.characters(outputChars, 0, outputChars.length);
        xmlHandler.endElement("", "", fieldName); //$NON-NLS-1$ //$NON-NLS-2$
    }
