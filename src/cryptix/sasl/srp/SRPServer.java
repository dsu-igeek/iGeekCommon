...
// $Id: SRPServer.java 8783 2013-06-27 16:55:51Z dave $
//
...
import cryptix.sasl.security.SaslServerExt;
...
implements SaslServerExt, SRPParams, SaslParams
{
   ...
   private byte[] sendNgL(byte[] input) throws SaslException {
      ...
      try { // Extract username
         username = frameIn.getText();
         cat.debug("Got U (client's username): \""+username+"\"");
      } catch (IOException x) {
         if (x instanceof SaslException)
            throw (SaslException) x;
   ...
   private byte[] sendEvidence(byte[] input) throws SaslException {
      ...
      byte[] k = K.getEncoded();
      byte[] expected = srp.generateClientEvidence(N, g, username, s, A, B, k, L);
      ...
      if (chosenConfidentialityAlgorithm == null) { // server MUST send M2
         ...
         byte[] M2 = srp.generateServerEvidence(A, M1, k, username, authorizationID, o);
         try {
            frameOut.setOS(M2);
         } catch (IOException x) {
            if (x instanceof SaslException)
